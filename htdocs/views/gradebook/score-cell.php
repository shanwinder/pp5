...
<div id="<?= $escape($cellId) ?>" data-score-cell data-save-state="<?= ($saved ?? false) ? 'saved' : 'idle' ?>">
  <input id="<?= $escape($cellId . '-input') ?>" class="form-control" name="score" type="text" inputmode="decimal" autocomplete="off"
  ...
  <small id="<?= $escape($cellId . '-status') ?>" role="status" aria-live="polite"><?= ($saved ?? false) ? 'บันทึกแล้ว' : '' ?></small>
</div>
